...
/// 同时支持Web和原生部署的Dio FormData
class YmFormData extends  FormData{
  ...
  /// The [files].
  final filesNew = <MapEntry<String,dynamic>>[];
  ...
  /// contain only ASCII characters.
  String _headerForFile(MapEntry<String, dynamic> entry) {
    dynamic file = null;
    if(entry.value is http.MultipartFile){
      file = entry.value as http.MultipartFile;
    }else if(entry.value is MultipartFile){
      file = entry.value as MultipartFile;
    }
  ...
  Stream<List<int>> finalize() {
    ...
    Future.forEach<MapEntry<String, dynamic>>(filesNew, (file) {
      writeAscii('--$boundary\r\n');

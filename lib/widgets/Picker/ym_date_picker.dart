...
class YmDatePickerState extends State<YmDatePicker> {
  ...
  void initState() {
    ...
    _data = [];
    //生成年的数据
    _data.add(List.generate(endDate.year - beginDate.year + 1,
        (index) => {"label": (beginDate.year + index).toString(), "value": (beginDate.year + index).toString()}));
    //生成月的数据
    _data.add(List.generate(
        12, (index) => {"label": YmDateUtil.format2Number((index + 1).toString()), "value": (index + 1).toString()}));
    //生成日的数据
    _data.add(List.generate(YmDateUtil.getDateCount(int.parse(_data[0][0]['value']), int.parse(_data[1][0]['value'])),
            ...
            child: Row(
                ...
                children: [
                  ...
                  YmTextButton("确定", textColor: Color(0xFF3446F2), onClick: () {
                    String date = _data[0][_currentIndex[0]]['value'] +
                        "-" +
                        YmDateUtil.format2Number(_data[1][_currentIndex[1]]['value']) +
                        "-" +
                        YmDateUtil.format2Number(_data[2][_currentIndex[2]]['value']);
                    print("选择的日期是：" + date);

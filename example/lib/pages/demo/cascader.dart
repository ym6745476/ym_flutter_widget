...
class _CascaderPageState extends State<CascaderPage> with SingleNativeStateMixin {
  ...
  Future<void> _changeData(Function dialogState, int pid, int index) async {
    ...
    dialogState(() {
      _regionData[index] = listNew;
      _regionIndex[index] =
          _regionIndex[index] > _regionData[index].length - 1 ? _regionData[index].length - 1 : _regionIndex[index];
    });
      ...
      builder: (context, state) {
        return YmCascader(
          ...
          onOkClick: () {
            Navigator.of(context).pop();
            setState(() {
              _chooseAddress = _regionData[0].elementAt(_regionIndex[0])['label'] +
                  "/" +
                  _regionData[1].elementAt(_regionIndex[1])['label'] +
                  "/" +
                  _regionData[2].elementAt(_regionIndex[2])['label'];
            });
          },
          onCancelClick: () {
            Navigator.of(context).pop();
